...
import com.bosscs.spark.jdbc.extractor.JdbcNativeCellExtractor;
import com.bosscs.spark.jdbc.extractor.JdbcNativeEntityExtractor;

import java.io.Serializable;
import java.util.List;
import java.util.Map;

import static com.bosscs.spark.commons.extractor.utils.ExtractorConstants.*;
 ...
 */
public class JdbcDeepJobConfig<T> extends JobConfig<T, JdbcDeepJobConfig<T>>
    ...
    public JdbcDeepJobConfig<T> driverClass(String driverClass) {
        try {
            ...
            throw new GenericException("Class " + driverClass + "not found", e);
        }
        return this;
    }
    ...
    public JdbcDeepJobConfig<T> table(String table) {
        this.table = table;
        if(dbSchema != null) {
            dbTable = new DbTable(dbSchema, table);
        }
        return this;
    }
    ...
    public JdbcDeepJobConfig<T> sort(String sort) {
        if(dbTable != null) {
            this.sort = new DbColumn(dbTable, sort, "",null);
        }
        return this;
    }
    ...
    public JdbcDeepJobConfig<T> partitionKey(String partitionKey) {
        if(dbTable != null) {
            this.partitionKey = new DbColumn(dbTable, partitionKey, "",null);
        }
        return this;
    }
        ...
        if(this.filters != null && this.filters.length > 0) {
            ...
            if (filters.length > 0) {
                for(int i=0; i<filters.length; i++) {
                    ...
                    DbColumn filterColumn = new DbColumn(dbTable, filter.getField(), "",null);
