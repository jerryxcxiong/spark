...
package com.bosscs.spark.mongodb.reader;
...
import com.bosscs.spark.commons.exception.ExtractorInitializationException;
import com.bosscs.spark.commons.impl.HadoopPartition;
import com.bosscs.spark.commons.rdd.IHadoopRecordReader;
import com.bosscs.spark.mongodb.config.MongoDeepJobConfig;
import com.bosscs.spark.mongodb.partition.MongoPartition;

/**
 * Created by Jerry Xiong on 30/02/16.
 *
 * @param <T> the type parameter
 */
public class MongoReader<T> implements IHadoopRecordReader<DBObject> {
    ...
    private DBObject createQueryPartition(MongoPartition partition) {
        ...
        if (partition.splitWrapper().getStartToken() != null) {
            queryBuilder.and(bsonObjectMin.toString());
        }

        if (partition.splitWrapper().getEndToken() != null) {
            queryBuilder.and(bsonObjectMax.toString());
        }
    ...
    private DBObject generateFilterQuery(MongoPartition partition) {
        ...
        if (mongoDeepJobConfig.getQuery() != null) {
            QueryBuilder queryBuilder = QueryBuilder.start();
            queryBuilder.and(createQueryPartition(partition).toString() + mongoDeepJobConfig.getQuery());
